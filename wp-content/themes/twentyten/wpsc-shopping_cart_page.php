...
if(wpsc_cart_item_count() < 1) :
	?>
    <div style="width:100%; margin:auto; border:solid 2px #000; padding:5px; font-size:15px; background-color:#ffffff; text-align:center; margin-top:30px;">    
   <?php _e('Oops, there is nothing in your cart.<br/><a href="http://digitalkickoff.com/parisgordon.com/?page_id=42">Please visit our shop</a>', 'wpsc') . "<a href=".get_option("product_list_url").">" . __('Please visit our shop', 'wpsc') . "</a>";
?>
</div>
<?php
return;
...
<div id="dots-border" style="margin-bottom:20px; width:95%; margin-top:10px; margin-right: 15px;"></div>
<table class="checkout_cart">
      ...
      <tr class="product_row product_row_<?php echo wpsc_the_cart_item_key(); ?> <?php echo $alt_class;?>">
         ...
         <td class="wpsc_product_quantity wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform qty">
               ...
               <input type="submit" value="<?php _e('', 'wpsc'); ?>" name="submit" style="background-image:url(<?php  echo get_bloginfo('template_directory');   ?>/images/update.png); border:none; width:65px; height:20px; cursor:pointer;" />
            </form>
         </td>
         ...
         <td class="wpsc_product_remove wpsc_product_remove_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform remove">
               ...
               <input type="hidden" name="key" value="<?php echo wpsc_the_cart_item_key(); ?>" />
               <input type="hidden" name="wpsc_update_quantity" value="true" />
               <input type="submit" value="<?php _e('', 'wpsc'); ?>" name="submit" style="background-image:url(<?php  echo get_bloginfo('template_directory');   ?>/images/remove.png); border:none; width:65px; height:20px; cursor:pointer;"  />
            </form>
         </td>
      </tr>
   <?php endwhile; ?>
  ...
  <?php if(wpsc_uses_shipping()): ?>
   ...
   <div id="wpsc_shopping_cart_container" style="width:900px; margin-top:15px; margin-left:auto; margin-right:auto;">
   <?php if(wpsc_uses_shipping()) : ?>
...
<table width="100%" border="0">
  <tr>
    <td><table class='wpsc_checkout_table table-1'>
      <?php $i = 0;
    ...
    <td> <table class='wpsc_checkout_table table-<?php echo $i; ?>'>
               <?php endif; ?>
            ...
            <?php
               ...
               "<div class='wpsc_email_address'>
                  ...
                  <p class='wpsc_email_address_p' style='height:76px;'>
...
?>
   ...
   <table  class='wpsc_checkout_table table-4' style="width:200px; margin-left:94px;">
      <?php if(wpsc_uses_shipping()) : ?>
	      <tr>
...
<!-- div for make purchase button -->
      <div class='wpsc_make_purchase' style="width:317px; margin-top:40px;">
         <span>
            <?php if(!wpsc_has_tnc()) : ?>
               <input type='hidden' value='yes' name='agree' />
            <?php endif; ?>
               <input type='hidden' value='submit_checkout' name='wpsc_action' />
               <input type='submit' value='<?php _e('', 'wpsc');?>' name='submit' class='make_purchase wpsc_buy_button' style="background-image:url(<?php  echo get_bloginfo('template_directory');   ?>/images/purchase.png); border:none; width:146px; height:42px;  cursor:pointer; background-color:#FED8E5;" />
         </span>
      </div>
