		...
		*/
		register_activation_hook(__FILE__, function() {
			...
			if (!get_option('pvd_options')) {
				add_option('pvd_options', array('interval' => 3600, 'ext' => 'txt'));
			}
			...
			$random = mt_rand(10000, 1000000000) + time();
			$key = substr(sha1(md5($random)), 0, 32);
	...
	*/
	...
	public function adminpage() {
		...
		<table cellspacing='10'>
			...
			<?php 
				if ($logs = array_reverse(get_option('pvd_logs'))) {
					foreach ($logs as $log) {
						echo '<tr>';
					...
					update_option('pvd_logs', array_slice($logs, 0, 20));
		...
		<h3>Usage</h3>
		Post via Dropbox checks automatically, through built-in cron function of WordPress, the existance of new files in your Dropbox folder and then it proceeds to update your blog. Once posted, the text files is moved into subidirectory "posted", if you have not selected "delete" option.
		<br />
		Text files must be uploaded inside <strong>Dropbox/Apps/Post_via_Dropbox/</strong> . The text file may have whatever extensions you want (default .txt) and it is strongly recommended that it has UTF-8 encoding. Why WordPress is able to read informations in proper manner, you must use some tags like <strong>[title] [/title]</strong> and <strong>[content] [/content]</strong>.
		<br /><br />
		This is the list of the tags that you can use:
		<ul>
		<li><strong>[title]</strong></strong> post title <strong><strong>[/title]</strong></strong> (mandatory) </li>
			<li> <strong>[content]</strong> the content of the post <strong>[/content]</strong> (mandatory) </li>
...
<li><strong>[ping]</strong> ping status (use word 'open' or 'closed') <strong>[/ping]</strong></li>
		...
		<br /><br />
		You can edit an existing post specifying the ID of the post. There're two ways: 1) using [id] tag or 2) prepend to filename the ID (example: 500-filename.txt).
The quickest way to edit an existing post, already posted via Dropbox, is move the file from the subfolder 'posted' to the principal folder.
		...
		foreach ($posts as $post) {
			...
			if ($id) { // post inserted
				...
				if ($options['delete']) {
					...
					try {
						$this->dropbox->move($post, '/posted/'.$id.'-'.pathinfo($post, PATHINFO_BASENAME));
					} catch (Exception $e) {
						$this->report_error($e->getMessage());
					}
				}
	...
	public function init() {
		...
		if ($results) {
			...
			update_option('pvd_logs', array_merge_recursive($logs, $results));
		}
